...
function check_action()
{
	...
	for (i=0;i<chk.length;i++)
	{
		...
		{
			sm_frm(document.getElementById('frmlist'),'<?php echo $this->site['base_url']?>admin_bbs/action/pid/<?php echo $mr['page_id']?>','<?php echo Kohana::lang('errormsg_lang.msg_confirm_update')?>');
			return true;
		}
	}
	...
	$.msgbox('<?php echo Kohana::lang('errormsg_lang.msg_confirm_del'); ?>', {
		...
		function(re) {
			if(re == 'Yes') {
				...
				$.getJSON("<?php echo $this->site['base_url']?>admin_<?php echo $this->mr['page_type_name']?>/delete/"+ id,
					function(obj) {
						if(obj.status) {
