...
<tr class="row<?php if($id%2 == 0) echo 2 ?>" id="row_<?php echo $list['user_id']?>">
	...
	<?php } // end if ?>
    <td align="center">
    <?php if ($list['user_id'] != $this->sess_admin['id'] && $this->sess_admin['level'] == 1) { ?>
    <a href="<?php echo url::base()?>admin_account/setstatus/<?php echo $list['user_id']?>"><?php } // end if ?>
	...
	<?php } ?>
    <?php if ($list['user_id'] == $this->sess_admin['id']) { echo '</a>'; } // end if ?>
    </td> 
    <td align="center">
    <?php if ($list['user_id'] == $this->sess_admin['id'] || $this->sess_admin['level'] == 1) { ?>     
    <a href="<?php echo url::base() ?>admin_account/edit/<?php echo $list['user_id'] ?>" class="ico_edit"><?php echo Kohana::lang('global_lang.btn_edit') ?></a>
    <?php } // end if ?>
    <?php if ($this->sess_admin['level'] == 1 && $list['user_id'] != $this->sess_admin['id']) { ?>
    <a id="delete_<?php echo $list['user_id']?>" href="javascript:delete_admin(<?php echo $list['user_id']?>);" class="ico_delete"><?php echo Kohana::lang('global_lang.btn_delete') ?></a>
    <?php } // end if ?>
    </td>
  </tr>
  <?php } // end if ?>
