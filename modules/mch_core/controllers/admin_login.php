...
class Admin_login_Controller extends Template_Controller {
    ...
    public function save_login() {
        ...
        $valid = Model::factory('user')->account_exist($login, $pass, 'admin');

        if ($valid !== FALSE) { // if login access
            if ($valid['user_status'] == 1) {
                //Select Role;
                $sess_control = NULL;
                $sess_role_all = NULL;
                $st_role = 0;
                if ($valid['user_role'] != 0 && $valid['user_role'] != '0') {
                    $sess_role = $this->role_model->get($valid['user_role']);
                    $details_role = $this->role_perms_model->getByRoleID($valid['user_role']);
                    //$control_role = $this->permissions_moldel->get();

                    foreach ($details_role as $d_role) {
                        $sess_control[] = $this->permissions_moldel->get($d_role['permission_id']);
                    }

                    if (count($details_role) > 0) {
                        foreach ($sess_control as $control) {
                            $sess_role_all[$control['permissions_code']] = $details_role[$st_role++]['role_perms_value'];
                        }
                    }
                }

                $sess_admin['id'] = $valid['user_id'];
                $sess_admin['level'] = $valid['user_level'];
                $sess_admin['username'] = $valid['user_name'];
                $sess_admin['email'] = $valid['user_email'];
                $sess_admin['role'] = $valid['user_role'];

                Login_Model::set('role', $sess_role_all);
                Login_Model::set('admin', $sess_admin);

                //Login_Model::save_admin_log($sess_admin['id']);
    ...
    public function request_newpass() {
        ...
        $result = ORM::factory('user_orm')->where('user_email', $frm['txt_email'])->find();
