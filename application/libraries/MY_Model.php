...
class Model extends Model_Core
{
	 ...
	 * @param $order string (asc/desc - default asc)
	 * @param $type_result arr | obj (type of result return)
	 *
	 * @access public
	 * @return array | object
	 **/
	public function get_mod($id = '', $type_result = 'arr', $order = 'asc')
	{
		if (!valid::digit($id) && $id !== '') return array();
		
		if ($id != '')
			$this->db->{is_array($id)?'in':'where'}($this->primary_key, $id);
		
		if (!empty($this->order_key)) $this->db->orderby($this->order_key, $order);
		
		$result = $this->db->get($this->table_name)->result_array($type_result=='arr' ? FALSE : TRUE);
		
		if ($id != '' && !is_array($id)) return (count($result)==1) ? $result[0] : array();
				
		$this->result_query = $result;
		return $result;
	}
	
	public function get_status($status, $id = '', $type_result = 'arr')
	{
		$this->db->where($this->status_key, $status);
		
		$this->result_query = $this->get_mod($id, $type_result);
		return $this->result_query;
	}
	
	public function get_last_rq()
	{
		return $this->result_query;
	}
