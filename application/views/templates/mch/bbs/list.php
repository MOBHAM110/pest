...
<tr>
    ...
    <tr>
        ...
        <?php 
        if(isset($mr['bbs_file']) && !empty($mr['bbs_file']))
        foreach ($mr['bbs_file'] as $file) { ?>
...
<tr><td class="bbs_list">
	...
	<?php if(isset($mlist) && !empty($mlist))
        foreach ($mlist as $id => $ml) { ?>
	<?php $expand = ''?>
	...
	<tr>
		...
		<?php if (!empty($this->sess_cus)) { ?>
		<td align="center">
		<?php if ((!empty($ml['bbs_author']) && $this->sess_cus['username'] == $ml['bbs_author']) || $this->sess_cus['level'] < 3) { ?>&nbsp;
